...
@app.get("/stocks")
async def stocks(session: session.SessionContainer = Depends(verify_session()), db: SessionLocal = Depends(get_db)): # type: ignore
...
@app.get("/all-stocks")
async def all_stocks(session: session.SessionContainer = Depends(verify_session()), db: SessionLocal = Depends(get_db)): # type: ignore
...
@app.post("/openai-key")
async def openai_key(request: Request, session: session.SessionContainer = Depends(verify_session()), db: SessionLocal = Depends(get_db)): # type: ignore
    user_id = session.get_user_id()
    data = await request.json()
    if "key" in data:
        dbc.update_openai_key(db, user_id, data["key"])
...
@app.get("/openai-key")
async def check_openai_key(session: session.SessionContainer = Depends(verify_session()), db: SessionLocal = Depends(get_db)): # type: ignore
    user_id = session.get_user_id()
    has_key = dbc.check_openai_key(db, user_id)
    return {"key": has_key}
